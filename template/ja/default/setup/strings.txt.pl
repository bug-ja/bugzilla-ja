...
* APACHE モジュール                                                   *
***********************************************************************
* いくつかの Apache モジュールにより Bugzilla の機能を拡張できます。  *
* これらのモジュールは Apache の設定ファイル (通常 httpd.conf もしく  *
* は apache2.conf です) にて有効化します。                            *
* - mod_headers, mod_env, mod_expires : ユーザのブラウザーキャッシュ  *
*   を Bugzilla 更新の際に自動的に更新する。                          *
* - mod_rewrite : REST API で利用している短い URL を有効化する。      *
* - mod_version : Apache 2.2 と 2.4 用の .htaccess に記述されている   *
*   設定を有効かする。                                                *
* 有効化する必要のあるモジュールは以下のものになります。              *
*                                                                     *
END
